...
class AtlanticClimate : public climate::Climate, public PollingComponent, public uart::UARTDevice {
 public:
  ...
  // Utile pour le reverse-engineering: expose via lambda YAML pour balayer l'espace des registres.
 ...
 protected:
  ...
  void debug_dump_frame_(const uint8_t *payload, uint8_t len);
  ...
  // Table de deduplication pour le sniff: cle = 6 octets packes (sender + 5 premiers octets du payload),
  // valeur = hash FNV-1a du payload. On ne re-log une trame que si son hash change.
  struct SniffSlot {
    uint64_t key;
    uint32_t hash;
  };
  SniffSlot sniff_slots_[16]{};
